...
public class KnightRule extends PieceRule {
    ...
    public Collection<ChessMove> getPossibleMoves(ChessBoard board, ChessPosition position) {
        ...
        if (position.getRow() > 0 && position.getColumn() < 6) {
            if (board.getPiece(new ChessPosition(position.getRow(), position.getColumn()+3)) == null ||
                    !board.getPiece(new ChessPosition(position.getRow(),
                            position.getColumn()+3)).getTeamColor().equals(teamColor)) {
        ...
        if (position.getRow() < 6 && position.getColumn() > 0) {
            if (board.getPiece(new ChessPosition(position.getRow()+3, position.getColumn())) == null ||
                    !board.getPiece(new ChessPosition(position.getRow()+3,
                            position.getColumn())).getTeamColor().equals(teamColor)) {
        ...
        if (position.getRow() < 6 && position.getColumn() < 7) {
            if (board.getPiece(new ChessPosition(position.getRow()+3, position.getColumn()+2)) == null ||
                    !board.getPiece(new ChessPosition(position.getRow()+3,
                            position.getColumn()+2)).getTeamColor().equals(teamColor)) {
                moves.add(new ChessMove(position, new ChessPosition(position.getRow()+3,
                        position.getColumn()+2)));
            }
        }
        if (position.getRow() < 7 && position.getColumn() < 6) {
            if (board.getPiece(new ChessPosition(position.getRow()+2, position.getColumn()+3)) == null ||
                    !board.getPiece(new ChessPosition(position.getRow()+2,
                            position.getColumn()+3)).getTeamColor().equals(teamColor)) {
                moves.add(new ChessMove(position, new ChessPosition(position.getRow()+2,
